...
template = """
 You are a marketing copywriter with 20 years of experience. You are analyzing customer's background to write personalized product description that only this customer will receive; 
    ...
    TASK: Write a product description that is tailored into this customer's Age group and status. Use age group specific slang.;
    ...
    USE CASE: write a story in 5 sentences, of an example weekend activity taking into account status {status} and age {agegroup}, write a story in first person, example "I started my Saturday morning with ...", OUTPUT TEXT in Estonian; 
"""
...
with col2:
    st.image(image='images.png', caption='Natural and healthy shirts for everybody')
...
def get_status():
    input_text = st.text_input(label="Customers main status", key="status_input")
    return input_text
...
#        st.stop()
    ...
    prompt_with_content = prompt.format(agegroup=option_agegroup, status=status_input, content=content_input)
